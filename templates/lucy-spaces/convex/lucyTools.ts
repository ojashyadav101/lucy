/**
 * Lucy Tools - Call any Lucy SDK function from your Convex app.
 *
...
const LUCY_SPACES_API_URL = process.env.LUCY_SPACES_API_URL!;
const PROJECT_NAME = process.env.LUCY_SPACES_PROJECT_NAME!;
const PROJECT_SECRET = process.env.LUCY_SPACES_PROJECT_SECRET!;

async function callTool<T>(role: string, args: Record<string, unknown> = {}): Promise<T> {
  const response = await fetch(`${LUCY_SPACES_API_URL}/api/lucy-spaces/tools/call`, {
    method: "POST",
