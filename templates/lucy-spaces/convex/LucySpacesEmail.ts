...
async function sendEmail({
  ...
  const apiUrl = process.env.LUCY_SPACES_API_URL;
  const projectName = process.env.LUCY_SPACES_PROJECT_NAME;
  const projectSecret = process.env.LUCY_SPACES_PROJECT_SECRET;

  if (!apiUrl || !projectName || !projectSecret) {
    throw new Error(
      "Lucy Spaces environment variables not configured. " +
        "Required: LUCY_SPACES_API_URL, LUCY_SPACES_PROJECT_NAME, LUCY_SPACES_PROJECT_SECRET",
    );
  }

  const response = await fetch(`${apiUrl}/api/lucy-spaces/send-email`, {
    method: "POST",
    headers: {
 ...
 * Sends OTP codes via Lucy Spaces API which:
 ...
 */
export const LucySpacesEmail = Email({
  id: "lucy-spaces-email",
 ...
 * Uses the same Lucy Spaces API but with different email template.
 */
export const LucySpacesPasswordReset = Email({
  id: "lucy-spaces-password-reset",
  maxAge: 60 * 15, // 15 minutes
