...
async def _make_request(
    ...
    try:
        async with httpx.AsyncClient(
            follow_redirects=True, timeout=settings.clerk_api_timeout_s
        ) as client:
            if method == "GET":
    ...
    for user in users:
        ...
        if isinstance(created, (int, float)):
            ...
            try:
                ...
                created = datetime.fromtimestamp(ts, tz=UTC).strftime("%Y-%m-%d %H:%M UTC")
            except Exception as e:
        ...
        if ext_accts and isinstance(ext_accts, list):
            providers = [a.get("provider", "") for a in ext_accts if isinstance(a, dict)]
            if providers:
...
async def clerk_get_user_stats(api_key: str, query_params: dict = None) -> dict:
    ...
    while True:
        ...
        for u in page:
            ...
            if ext and isinstance(ext, list):
                providers = {a.get("provider", "unknown") for a in ext if isinstance(a, dict)}
                for p in providers:
            ...
            if isinstance(created, (int, float)):
                ...
                try:
                    ts = created / 1000 if created > 1e12 else created
                    month_key = datetime.fromtimestamp(ts, tz=UTC).strftime("%Y-%m")
    ...
    return {
        "total_users": total,
        "auth_method_breakdown": dict(sorted(auth_methods.items(), key=lambda x: -x[1])),
...
async def clerk_update_organization(
    api_key: str, organization_id: str, organization_data: dict
) -> dict:
    """
    Updates an existing organization's information.
    """
    return await _make_request(
        "PATCH", f"organizations/{organization_id}", api_key, json_data=organization_data
    )
...
async def clerk_list_organization_memberships(
    api_key: str, organization_id: str, query_params: dict = None
) -> dict:
    """
    Lists all memberships for a given organization.
    """
    return await _make_request(
        "GET", f"organizations/{organization_id}/memberships", api_key, params=query_params
    )
...
async def clerk_delete_organization_membership(
    ...
    """
    return await _make_request(
        "DELETE", f"organizations/{organization_id}/memberships/{user_id}", api_key
    )


# --- Webhook Management (Simulated - Clerk's backend API doesn't directly manage webhooks via API) ---  # noqa: E501
...
async def clerk_list_webhooks(api_key: str) -> dict:
    ...
    """
    return {
        "error": "Clerk's backend API does not directly manage webhooks via API. Please configure webhooks in the Clerk Dashboard."  # noqa: E501
    }
...
TOOLS: list[dict[str, Any]] = [
                ...
                "query_params": {
                    ...
                    "properties": {
                        ...
                        "created_after_unix_ms": {
                            "type": "integer",
                            "description": "Only return users created AFTER this Unix timestamp in MILLISECONDS. Example: 1708992000000 for Feb 27 2024. Use this to count signups in a time period.",  # noqa: E501
                        },
                        "created_before_unix_ms": {
                            "type": "integer",
                            "description": "Only return users created BEFORE this Unix timestamp in MILLISECONDS.",  # noqa: E501
                        },
                        "limit": {
                            "type": "integer",
                            "description": "Number of records to return. Max 500. Use 100 for comprehensive listing.",  # noqa: E501
                            "default": 100,
                        },
                        "offset": {
                            "type": "integer",
                        ...
                        "order_by": {
                            "type": "string",
                            "description": "Order results by a specific field. Use '-created_at' for newest first, 'created_at' for oldest first.",  # noqa: E501
                        },
                        "return_all": {
                            "type": "string",
                            "description": "Set to 'true' to return ALL users as JSON. May be large for 1000+ users.",  # noqa: E501
                            "default": "false",
                        },
                        "export_excel": {
                            "type": "string",
                            "description": "Set to 'true' to generate a formatted Excel file with ALL users. Returns a file path you can upload to Slack. Preferred for reports and exports.",  # noqa: E501
                            "default": "false",
                        },
                    },
                }
    ...
    },
    {
        ...
        "description": "Creates a new user in your Clerk instance. Requires at least one of `email_address`, `phone_number`, or `username`.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "user_data": {
                    ...
                    "properties": {
                        ...
                        "password": {
                            "type": "string",
                            "description": "Password for the user (if not using passwordless).",
                        },
                        ...
                        "public_metadata": {
                            "type": "object",
                            "description": "Public metadata for the user.",
                        },
                        "private_metadata": {
                            "type": "object",
                            "description": "Private metadata for the user.",
                        },
                        "unsafe_metadata": {
                            "type": "object",
                            "description": "Unsafe metadata for the user.",
                        },
                        "skip_password_checks": {
                            "type": "boolean",
                            "description": "Whether to skip password strength checks.",
                            "default": False,
                        },
                        "skip_email_verification": {
                            "type": "boolean",
                            "description": "Whether to skip email verification for new email addresses.",  # noqa: E501
                            "default": False,
                        },
                        "skip_phone_number_verification": {
                            "type": "boolean",
                            "description": "Whether to skip phone number verification for new phone numbers.",  # noqa: E501
                            "default": False,
                        },
                    },
                    "minProperties": 1,
    ...
    },
    {
        ...
        "description": "Updates an existing user's information. Only provide fields that need to be updated.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "user_id": {
                ...
                "user_data": {
                    ...
                    "properties": {
                        ...
                        "public_metadata": {
                            "type": "object",
                            "description": "Public metadata to merge.",
                        },
                        "private_metadata": {
                            "type": "object",
                            "description": "Private metadata to merge.",
                        },
                        "unsafe_metadata": {
                            "type": "object",
                            "description": "Unsafe metadata to merge.",
                        },
                        ...
                        "backup_code_enabled": {
                            "type": "boolean",
                            "description": "Whether backup codes are enabled for the user.",
                        },
                    },
                    "minProperties": 1,
                },
    ...
    {
        "name": "clerk_ban_user",
        "description": "Bans a user, preventing them from signing in. This does not delete the user.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "user_id": {
    ...
    },
    {
        ...
        "description": "Lists all organizations in your Clerk instance. Supports filtering by `query` (name or slug) and pagination with `limit` and `offset`.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "query_params": {
                    ...
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": "Filter organizations by name or slug.",
                        },
                        "limit": {
                            "type": "integer",
                        ...
                        "order_by": {
                            "type": "string",
                            "description": "Order results by a specific field (e.g., 'created_at', '-created_at').",  # noqa: E501
                        },
                    },
                }
            },
            ...
            "properties": {
                "organization_data": {
                    ...
                    "properties": {
                        ...
                        "slug": {
                            "type": "string",
                            "description": "The slug of the organization (unique identifier).",
                        },
                        "created_by": {
                            "type": "string",
                        ...
                        "unsafe_metadata": {
                            "type": "object",
                            "description": "Unsafe metadata for the organization.",
                        },
                        "image_url": {
                            "type": "string",
                            "format": "uri",
    ...
    },
    {
        ...
        "description": "Updates an existing organization's information. Only provide fields that need to be updated.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "organization_id": {
                ...
                "organization_data": {
                    ...
                    "properties": {
                        ...
                        "image_url": {
                            "type": "string",
                            "format": "uri",
                            "description": "New URL of the organization's image.",
                        },
                    },
                    "minProperties": 1,
                },
    ...
    },
    {
        ...
        "description": "Lists all memberships for a given organization. Supports pagination with `limit` and `offset`.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "organization_id": {
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "user_id": {
                    "type": "string",
                    "description": "The ID of the user whose membership to update.",
                    "pattern": "^user_[a-zA-Z0-9]+$",
                },
                "role": {
                    "type": "string",
                    "description": "The new role for the user in the organization (e.g., 'admin', 'member').",  # noqa: E501
                    "enum": ["admin", "member"],
                },
            },
    ...
    },
    {
        ...
        "description": "Lists all active sessions. Can filter by `user_id`. Supports pagination with `limit` and `offset`.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "query_params": {
                    ...
                    "properties": {
                        ...
                        "status": {
                            "type": "string",
                            "description": "Filter sessions by status (e.g., 'active', 'revoked').",
                        },
                        "limit": {
                            "type": "integer",
                            "description": "Number of records to return. Max 500.",
    ...
    },
    {
        ...
        "description": "Revokes a specific session, effectively logging out the user associated with it.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "session_id": {
    ...
    },
    {
        ...
        "description": "Lists all email addresses. Can filter by `user_id`. Supports pagination with `limit` and `offset`.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "query_params": {
                    ...
                    "properties": {
                        "user_id": {
                            "type": "string",
                            "description": "Filter email addresses by user ID.",
                            "pattern": "^user_[a-zA-Z0-9]+$",
                        },
                        "limit": {
                            "type": "integer",
    ...
    },
    {
        ...
        "description": "Lists all phone numbers. Can filter by `user_id`. Supports pagination with `limit` and `offset`.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "query_params": {
                    ...
                    "properties": {
                        "user_id": {
                            "type": "string",
                            "description": "Filter phone numbers by user ID.",
                            "pattern": "^user_[a-zA-Z0-9]+$",
                        },
                        "limit": {
                            "type": "integer",
    ...
    },
    {
        ...
        "description": "(Simulated) Lists all configured webhooks. In a real Clerk setup, webhooks are configured via the dashboard, and this tool is for conceptual understanding.",  # noqa: E501
        "parameters": {"type": "object", "properties": {}},
    },
    {
        "name": "clerk_create_webhook",
        "description": "(Simulated) Creates a new webhook endpoint. In a real Clerk setup, webhooks are configured via the dashboard. This tool is for conceptual understanding.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
                "webhook_data": {
                    ...
                    "properties": {
                        ...
                        "events": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "List of event types to subscribe to (e.g., 'user.created', 'organization.created').",  # noqa: E501
                        },
                        "secret": {
                            "type": "string",
    ...
    },
    {
        ...
        "description": "(Simulated) Deletes a webhook endpoint. In a real Clerk setup, webhooks are configured via the dashboard. This tool is for conceptual understanding.",  # noqa: E501
        "parameters": {
            "type": "object",
            "properties": {
